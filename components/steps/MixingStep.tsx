...
export function MixingStep({ txHash, onReady, onCancel }: { txHash: string | null; onReady: () => void; onCancel: () => void }) {
  ...
  const fmt = (ms: number) => { const s = Math.floor(ms / 1000); return `${Math.floor(s / 60)}m ${(s % 60).toString().padStart(2, "0")}s`; };
  ...
  return (
    <div>
      ...
      <div className="relative overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/50 p-5 mb-4">
        ...
        <div className="flex justify-between text-[10px] text-zinc-700 mt-2"><span>0m</span><span>60m</span></div>
      </div>

      {txHash && (
        <a href={`https://arbiscan.io/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-zinc-600 hover:text-pink-400 transition-colors mb-4">
          <ArrowSquareOut size={12} weight="bold" />View deposit on explorer
        </a>
      )}

      <div className="rounded-xl border border-zinc-800 px-4 py-3">
        <p className="text-xs text-zinc-500">You can close this tab and come back — progress is saved locally. Cancel below to return funds to your wallet.</p>
      </div>
    </div>
  );
}
