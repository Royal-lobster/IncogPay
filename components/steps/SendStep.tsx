...
export function SendStep({
  ...
  const isValid =
    ...
    parseFloat(sendAmount) <= available;
  ...
  return (
    <div>
      ...
      <p className="text-sm text-zinc-400 mb-4">
        Enter the recipient's address. A ZK proof will be generated — takes ~30 seconds.
      </p>

      <div className="flex justify-between text-xs px-1 mb-2">
        <span className="text-zinc-600">Available</span>
        <span className="text-zinc-400">
          {available.toFixed(2)} {intent.token}
        </span>
      </div>
      ...
      <button
        ...
        className={`w-full py-2.5 rounded-full text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${!isValid || busy ? "bg-zinc-800 text-zinc-600 cursor-not-allowed" : "bg-white text-black hover:bg-zinc-200"}`}
      >
