...
const steps = [
  ...
  { icon: PaperPlaneTilt, role: "Step 3", title: "Enter recipient & send", desc: "Enter the destination address and confirm. No native token needed — relayer handles gas.", time: "~1 min" },
];
  ...
  return (
    <div>
          ...
          <div key={i} className="relative overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
            ...
            <div className="flex gap-4">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-pink-500/10 ring-1 ring-pink-500/20">
                <s.icon size={15} weight="duotone" className="text-pink-400" />
              </div>
      ...
      <div className="rounded-xl border border-zinc-800 px-4 py-3 mb-6 flex gap-3">
        <ArrowCounterClockwise size={13} weight="bold" className="text-zinc-600 mt-0.5 shrink-0" />
      ...
      <div className="flex gap-3">
        <button onClick={onCancel} className="flex-1 py-3 rounded-full border border-zinc-700 text-sm text-zinc-400 hover:text-zinc-200 hover:border-zinc-500 transition-colors">Cancel</button>
        <button onClick={onStart} className="flex-1 py-3 rounded-full bg-white text-black text-sm font-semibold hover:bg-zinc-200 transition-colors">Start →</button>
      </div>
    </div>
  );
}
